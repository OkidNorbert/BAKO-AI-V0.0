...
async def get_ui():
    ...
    <body>
        <div class="container">
            ...
            <div id="batch" class="tab-content">
                <div class="section">
                    ...
                    <div class="grid">
                        <div class="card">
                            <h3>Upload Multiple Videos</h3>
                            <div class="file-upload" id="batch-video-upload" onclick="document.getElementById('batch-video-files').click()">
                                <p>📹 Click to upload multiple videos</p>
                                <p class="small">Select multiple .mp4, .avi, .mov files</p>
                            </div>
                            <input type="file" id="batch-video-files" multiple accept="video/*" style="display: none;" onchange="handleBatchVideoUpload(event)">
                            <div id="batch-video-list" class="file-list"></div>
                        </div>
                        ...
                        <div class="card">
                            ...
                            <div class="form-group">
                                ...
                                <select id="batch-mode" onchange="updateBatchMode()">
                                    <option value="regular">Regular Extraction</option>
                                    <option value="area">Area-Specific Extraction</option>
                                </select>
                            </div>
                            
                            <div id="batch-regular-options">
                                <div class="form-group">
                                    <label>Extraction Strategy:</label>
                                    <select id="batch-strategy">
                                        <option value="uniform">Uniform (every Nth frame)</option>
                                        <option value="temporal">Temporal (distributed)</option>
                                        <option value="motion">Motion-based</option>
                                        <option value="all">All Strategies</option>
                                    </select>
                                </div>
                            ...
                            <div id="batch-area-options" style="display: none;">
                                <div class="form-group">
                                    <label>Select Areas:</label>
                                    <div class="checkbox-group">
                                        <label><input type="checkbox" id="batch-area-ball" checked> 🏀 Ball</label>
                                        <label><input type="checkbox" id="batch-area-player" checked> 👤 Player</label>
                                        <label><input type="checkbox" id="batch-area-court" checked> 📏 Court Lines</label>
                                        <label><input type="checkbox" id="batch-area-hoop" checked> 🏀 Hoop</label>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <label>
                                    ...
                                    Process videos in parallel (faster)
                                </label>
                            </div>
                            
                            <div class="form-group">
                                <label>Max Workers (for parallel):</label>
                                <input type="number" id="batch-workers" value="4" min="1" max="8">
                            </div>
                            
                            <div class="form-group">
                                <label>Max Frames Per Video:</label>
                                <input type="number" id="batch-max-frames" value="100" min="10" max="1000">
                            </div>
                            
                            <button class="btn" onclick="processBatchVideos()">🚀 Process Batch</button>
                        </div>
                    </div>
                    <div id="batch-results" class="status" style="display: none;"></div>
                </div>
            </div>
        ...
        <script>
            ...
            // Batch video upload handling
            function handleBatchVideoUpload(event) {
                const files = Array.from(event.target.files);
                currentBatchVideos = files.map(f => f.name);
                
                const listDiv = document.getElementById('batch-video-list');
                listDiv.innerHTML = '';
                ...
                if (files.length > 0) {
                    files.forEach((file, index) => {
                        ...
                        div.innerHTML = `${index + 1}. ${file.name} (${(file.size / 1024 / 1024).toFixed(2)} MB)`;
                        listDiv.appendChild(div);
                    });
                    showStatus('batch-results', `${files.length} videos selected`, 'success');
                }
            }
            ...
            function updateBatchMode() {
                ...
                const regularOptions = document.getElementById('batch-regular-options');
                const areaOptions = document.getElementById('batch-area-options');
            ...
            async function processBatchVideos() {
                if (currentBatchVideos.length === 0) {
                    showStatus('batch-results', 'Please select at least one video file', 'error');
                    return;
                }
                
                const mode = document.getElementById('batch-mode').value;
                const parallel = document.getElementById('batch-parallel').checked;
                const maxWorkers = parseInt(document.getElementById('batch-workers').value);
                const maxFrames = parseInt(document.getElementById('batch-max-frames').value);
                
                showStatus('batch-results', `Processing ${currentBatchVideos.length} videos...`, 'info');
                
                try {
                    let requestBody = {
                        video_paths: currentBatchVideos,
                        output_dir: 'training/datasets/basketball/batch_frames',
                        parallel: parallel,
                        max_workers: maxWorkers,
                        max_frames_per_area: maxFrames
                    };
                    
                    if (mode === 'area') {
                        // Area-specific batch processing
                        const selectedAreas = [];
                        if (document.getElementById('batch-area-ball').checked) selectedAreas.push('ball');
                        if (document.getElementById('batch-area-player').checked) selectedAreas.push('player');
                        if (document.getElementById('batch-area-court').checked) selectedAreas.push('court_lines');
                        if (document.getElementById('batch-area-hoop').checked) selectedAreas.push('hoop');
                        
                        if (selectedAreas.length === 0) {
                            showStatus('batch-results', 'Please select at least one basketball area', 'error');
                            return;
                        }
                        ...
                        requestBody.strategy = document.getElementById('batch-strategy').value;
                    }
                    ...
                    if (result.success) {
                        ...
                        message += `Total Frames: ${result.total_frames || 0}`;
                        
                        if (result.areas_requested) {
                            message += `\nAreas: ${result.areas_requested.join(', ')}`;
                            if (result.area_totals) {
                                message += '\nFrames per area:';
                                for (const [area, count] of Object.entries(result.area_totals)) {
                                    message += `\n  ${area}: ${count}`;
                                }
                            }
                        }
                        
                        showStatus('batch-results', message, 'success');
                    } else {
                        showStatus('batch-results', result.message, 'error');
                    }
                } catch (error) {
                    showStatus('batch-results', `Error: ${error.message}`, 'error');
                }
            }
