...
const Analytics = () => {
  ...
  // Training trend chart data
  const sessionsChartData = {
    labels: analyticsData.sessions.trend.map(item => item.date),
    datasets: [
      {
        label: 'Sessions',
        data: analyticsData.sessions.trend.map(item => item.value),
        borderColor: '#3b82f6',
  ...
  const skillsChartData = {
    labels: analyticsData.skills.byType.map(item => item.type),
    datasets: [
      {
        label: 'Skill Distribution',
        data: analyticsData.skills.byType.map(item => item.amount),
        backgroundColor: [
          'rgba(59, 130, 246, 0.7)',
          'rgba(34, 197, 94, 0.7)',
          'rgba(249, 115, 22, 0.7)',
    ...
    <div className={`min-h-screen p-6 ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      <div className="max-w-7xl mx-auto">
          ...
          <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{analyticsData.sessions.total}</p>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          {/* Sessions Trend */}
          <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <h2 className="text-lg font-semibold mb-4">Training Trend</h2>
            <div className="h-64">
              {loading ? (
          ...
          {/* Skill Distribution */}
          <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <h2 className="text-lg font-semibold mb-4">Skill Focus Distribution</h2>
            <div className="h-64">
              {loading ? (
                <div className="flex items-center justify-center h-full">
                ...
                <Pie data={skillsChartData} options={{
                  ...chartOptions,
                  maintainAspectRatio: false,
                  plugins: {
