    ...
    try {
      ...
      switch (activeTab) {
        ...
        case 'contacts':
          ...
          break;
        case 'players':
          response = await api.get('/admin/players');
  ...
  const getPaginatedData = () => {
    ...
    switch (activeTab) {
      case 'coachs':
        ...
        break;
      case 'contacts':
        filteredData = filterData(contacts);
        break;
      case 'players':
        filteredData = filterData(players);
        break;
      default:
        filteredData = [];
    }
        ...
        <div className={`rounded-lg p-6 max-w-2xl w-full ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold">
              {activeTab === 'players' ? 'Player Details' : 
               activeTab === 'contacts' ? 'Contact Details' : 'Coach Details'}
            </h2>
            <button
          ...
          <div className={`overflow-y-auto max-h-[70vh] p-4 rounded-md ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
            ...
            {/* Special handling for players or other relations */}
            {activeTab === 'contacts' && selectedItem.players && (
              <div className="mt-4">
                <h3 className="text-lg font-medium mb-2">Players</h3>
                {selectedItem.players.length > 0 ? (
                  <ul className="space-y-2">
                    {selectedItem.players.map((player, index) => (
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        ...
                        <div className="text-sm text-gray-500">
                          {activeTab === 'players' ? `Age: ${item.age || 'N/A'}` : null}
                        </div>
                      </div>
                    </div>
                  </td>
            ...
            <div>
              <p className="text-sm text-gray-500">
                ...
                <span className="font-medium">{Math.min(page * itemsPerPage, 
                  (activeTab === 'coachs' ? coachs : activeTab === 'contacts' ? contacts : players).length)}</span> of{' '}
                <span className="font-medium">{
                  (activeTab === 'coachs' ? coachs : activeTab === 'contacts' ? contacts : players).length
                }</span> results
              </p>
            </div>
            <div className="flex space-x-2">
